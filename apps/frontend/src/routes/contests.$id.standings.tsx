...
export default function ContestStandingsPage() {
  ...
  return (
    <div className="mx-auto max-w-[1600px] px-4 py-8 md:px-6 md:py-10 stagger">
      <header className="mb-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="font-display text-2xl sm:text-3xl md:text-4xl font-black tracking-tight flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-3">
              Ranking of <span className="text-primary break-words">{contest?.name || "Contest"}</span>
            </h1>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="relative group flex-1 md:flex-none md:shrink-0">
              ...
              <input 
                ...
                className="h-10 w-full md:w-64 pl-10 pr-4 rounded-xl bg-muted/30 border border-border/40 focus:outline-none focus:ring-2 ring-primary/20 focus:bg-muted/50 transition-all text-sm font-medium"
              />
            </div>
        ...
        <table className="w-full border-collapse text-left text-sm">
          <thead>
            <tr className="border-b border-border/40 bg-muted/20 backdrop-blur-md">
              <th className="px-3 py-3 sm:px-6 sm:py-5 font-black uppercase tracking-widest text-[10px] text-muted-foreground w-20">Rank</th>
              <th className="px-3 py-3 sm:px-6 sm:py-5 font-black uppercase tracking-widest text-[10px] text-muted-foreground">Name</th>
              <th className="px-3 py-3 sm:px-6 sm:py-5 font-black uppercase tracking-widest text-[10px] text-muted-foreground text-center">Score</th>
              <th className="px-3 py-3 sm:px-6 sm:py-5 font-black uppercase tracking-widest text-[10px] text-muted-foreground text-center">Finish Time</th>
              {problems.map((p: any) => (
                <th key={p.id} className="px-3 py-3 sm:px-6 sm:py-5 font-black uppercase tracking-widest text-[10px] text-muted-foreground text-center min-w-[72px] sm:min-w-[120px]">
                  {p.label} ({p.points})
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-border/20">
            {paginatedStandings.map((row: any) => (
              <tr key={row.user.id} className={cn(
                ...
                <td className="px-3 py-3 sm:px-6 sm:py-4">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="font-black text-sm group-hover:text-primary transition-colors truncate">{row.user.username || row.user.name}</div>
                ...
                <td className="px-3 py-3 sm:px-6 sm:py-4 text-center font-mono font-black text-primary text-base">
                  {row.totalPoints}
                </td>
                <td className="px-3 py-3 sm:px-6 sm:py-4 text-center font-mono text-xs font-bold text-muted-foreground">
                  {formatTime(row.totalTime)}
                </td>
                {problems.map((cp: any) => {
                  const solved = row.solvedProblems.find((s: any) => s.label === cp.label);
                  return (
                    <td key={cp.id} className="px-3 py-3 sm:px-6 sm:py-4">
                      {solved ? (
