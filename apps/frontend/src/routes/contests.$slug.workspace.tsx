...
export default function ContestWorkspace() {
  ...
  // Below lg the description/editor split must stack vertically — a horizontal
  // split is unusable when each panel is only ~150px wide on a phone.
  ...
  if (!isAdmin) {
    if (!isRegistered) {
      return (
        <div className="min-h-dvh w-full flex flex-col items-center justify-center bg-background p-6 text-center stagger">
          <div className="relative mb-8">
             <div className="absolute -inset-4 sm:-inset-10 bg-primary/20 blur-3xl rounded-full" />
             <Trophy className="h-20 w-20 sm:h-24 sm:w-24 text-primary/40 relative" />
          </div>
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight">Access Restricted</h1>
    ...
    if (!hasStarted) {
      return (
        <div className="min-h-dvh w-full flex flex-col items-center justify-center bg-background p-6 text-center stagger">
          <div className="relative mb-8">
             <div className="absolute -inset-4 sm:-inset-10 bg-primary/20 blur-3xl rounded-full" />
             <Clock className="h-20 w-20 sm:h-24 sm:w-24 text-primary/40 relative" />
          </div>
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight">Arena Not Open</h1>
          <p className="mt-4 text-muted-foreground max-w-md mx-auto leading-relaxed">
  ...
  return (
    <div className="h-dvh w-full flex flex-col bg-background overflow-hidden selection:bg-primary/30 relative">

      {/* Contest Header - Hidden in Full Screen */}
      {/* Contest Header */}
      <header className="h-14 border-b border-border bg-card/50 backdrop-blur-md flex items-center justify-between gap-2 px-3 sm:px-4 shrink-0 z-50 shadow-sm">
          <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
            <Link to={`/contests/${id}`} className="flex items-center gap-1.5 sm:gap-2 group shrink-0">
              <ArrowLeft className="h-4 w-4 text-muted-foreground group-hover:text-primary transition-colors" />
              <Trophy className="h-4 w-4 text-primary" />
              <span className="font-display font-bold text-sm truncate max-w-[80px] sm:max-w-[160px] lg:max-w-[200px]">{contest.name}</span>
            </Link>
            ...
            <div className="flex items-center gap-2 min-w-0 sm:ml-2">
              ...
              <select 
                ...
                className="max-w-[140px] sm:max-w-[220px] lg:max-w-none truncate bg-muted/40 border border-border/60 rounded-lg px-2 sm:px-3 py-1.5 text-xs sm:text-sm font-bold focus:ring-2 ring-primary outline-none transition-all cursor-pointer hover:bg-muted/60"
              >
                {problems.map(cp => (
          ...
          <div className="flex items-center gap-2 sm:gap-4 shrink-0">
            ...
            <Button size="sm" variant="outline" className="rounded-xl border-primary/20 hover:bg-primary/5 text-primary px-2.5 sm:px-3" onClick={() => navigate(`/contests/${id}/standings`)}>
              <LayoutDashboard className="h-4 w-4 sm:mr-2" /> <span className="hidden sm:inline">Standings</span>
            </Button>
          </div>
      </header>

      <main className="flex-1 flex overflow-hidden relative bg-background">
        <ResizablePanelGroup direction={isStacked ? "vertical" : "horizontal"}>
            <ResizablePanel defaultSize={40} minSize={20} id="description-panel">
              <div className="h-full overflow-y-auto scrollbar-thin bg-background p-4 md:p-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="min-w-0">
                    <h1 className="font-display text-lg sm:text-xl md:text-2xl font-bold">{activeProblem?.problem?.title || "Select a problem"}</h1>
            ...
            <ResizableHandle className={cn("bg-border hover:bg-primary/50 transition-colors", isStacked ? "h-1 w-full" : "w-1")} />
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className={cn(
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-row items-center gap-2 sm:flex-col sm:items-end sm:gap-1 font-mono text-[10px] text-muted-foreground uppercase tracking-widest font-bold">
            <div className="flex items-center gap-2">
