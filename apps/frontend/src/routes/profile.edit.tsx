          ...
          {socials.map((s, i) => (
            <div key={i} className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <select
                ...
                className="w-full sm:w-auto sm:min-w-[120px] shrink-0 rounded-md border border-input bg-background px-3 py-1.5 sm:py-1 text-sm"
              >
              ...
              <div className="flex gap-2 min-w-0 flex-1">
                <Input value={s.url} onChange={(e) => updateSocial(i, { url: e.target.value })} placeholder="https://..." className="min-w-0 flex-1" />
                <Button onClick={() => removeSocial(i)} variant="ghost" size="icon" className="shrink-0"><X className="h-3 w-3" /></Button>
              </div>
            </div>
          ))}
        </div>
