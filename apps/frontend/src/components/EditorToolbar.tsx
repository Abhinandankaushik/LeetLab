...
export function EditorToolbar({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-primary/10 bg-background/40 px-2 sm:px-3 py-2 backdrop-blur-xl sticky top-0 z-30 shadow-sm">
        ...
        {problems && onProblemChange && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="sm" className="h-8 gap-2 font-black text-xs hover:bg-primary/10 border border-primary/20 bg-primary/5 rounded-lg px-2 sm:px-3 min-w-0 max-w-[55vw] sm:max-w-none">
                <Trophy className="h-3.5 w-3.5 text-primary shrink-0" />
                <span className="text-primary truncate max-w-[120px] sm:max-w-[220px] md:max-w-none">{activeProblem?.label}. {activeProblem?.problem?.title}</span>
              </Button>
            </DropdownMenuTrigger>
