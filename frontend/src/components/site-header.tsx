...
import {
  ...
  ListChecks, Layers, Activity, Shield, Sparkles, ChevronDown, Home as HomeIcon
} from "lucide-react";
          ...
          {user ? (
            <div className="relative" ref={profileRef}>
              <button
                ...
                className="group flex items-center gap-2 rounded-full border border-border/60 bg-card pr-3 pl-1 py-1 transition-all hover:border-primary hover:shadow-[0_0_20px_var(--glow)]"
                aria-haspopup="menu"
              ...
              {profileOpen && (
                <div
                  ...
                  className="absolute right-0 top-full mt-2 w-72 origin-top-right animate-scale-in rounded-xl border border-border bg-popover shadow-2xl"
                  role="menu"
                >
                  ...
                  <div className="p-1">
                    <DropItem to="/profile" icon={UserIcon}>My Profile</DropItem>
                    <DropItem to="/profile/edit" icon={Settings}>Edit Profile</DropItem>
                    <DropItem to="/playlists" icon={Layers}>Playlists</DropItem>
                    <DropItem to="/submissions" icon={ListChecks}>Submissions</DropItem>
                  </div>
                  ...
                  <div className="border-t border-border/60 p-1 lg:hidden">
                    ...
                    {SECONDARY.map((it) => (
                      <DropItem key={it.to} to={it.to} icon={it.icon}>{it.label}</DropItem>
                    ))}
                  </div>

                  {user.role === "ADMIN" && (
                    <div className="border-t border-border/60 p-1">
                      <DropItem to="/admin" icon={Shield} accent>Admin Dashboard</DropItem>
                      <DropItem to="/admin/problems/ai" icon={Sparkles}>New Problem (AI)</DropItem>
                    </div>
                  )}
...
function DropItem({ to, icon: Icon, children, accent = false }: { to: string; icon?: any; children: React.ReactNode; accent?: boolean }) {
  return (
    <Link
      to={to}
      className={[
