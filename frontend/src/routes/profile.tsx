        ...
        <main className="space-y-6 min-w-0">
           ...
           <div className="rounded-2xl border border-border bg-card overflow-hidden shadow-sm">
              <div className="flex border-b border-border">
                 <button className="px-6 py-4 text-xs font-bold uppercase tracking-widest border-b-2 border-primary text-primary bg-primary/5">Recent AC</button>
                 <button className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-muted-foreground hover:bg-muted/30">Submissions</button>
                 <button className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-muted-foreground hover:bg-muted/30">Solutions</button>
              </div>
              <div className="divide-y divide-border/40">
                 {allSubmissions.map((s: any) => (
                   <Link key={s.id} to={`/problems/${s.problem.id}`} className="flex items-center justify-between p-4 hover:bg-muted/30 transition-all group">
                      <div className="flex items-center gap-3">
                         <div className={cn(
                           "h-2 w-2 rounded-full",
                           s.status === 'Accepted' ? "bg-easy" : "bg-hard"
                         )} />
                         <span className="font-bold group-hover:text-primary transition-colors">{s.problem.title}</span>
                      </div>
                      <span className="text-[10px] font-mono text-muted-foreground uppercase">{new Date(s.createdAt).toLocaleDateString()}</span>
                   </Link>
                 ))}
